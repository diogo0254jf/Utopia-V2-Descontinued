	...
	if steam then
		if not vRP.isBanned(steam) then
			if vRP.isWhitelisted(steam) then
				...
				deferrals.done("[Creative] Você não possui visto na cidade! [ Retire seu visto em: https://discord.gg/67ewqmkkeN ][ "..steam.." ]")
				TriggerEvent("queue:playerConnectingRemoveQueues",ids)
			end
		else
			deferrals.done("[Creative] Você foi banido da cidade. [ "..steam.." ]")
			TriggerEvent("queue:playerConnectingRemoveQueues",ids)
		end
	end
end)
...
AddEventHandler("baseModule:idLoaded",function(source,user_id,model)
	...
	if vRP.rusers[user_id] == nil then
		...
		if model ~= nil then
			TriggerClientEvent("Notify",source,"azul","Seja bem-vindo ao <b>Creative Rp</b>.<br>A cidade foi desenvolvida pensando especialmente em voce, desenvolva sua historia e nao esqueca que deixamos separado para voce um agrado em seu banco.",15000)
			...
			vRP.user_tables[user_id].inventorys["6"] = { item = "dollars", amount = 2000 }
		end
...
AddEventHandler("vRP:playerSpawn",function(user_id,source)
	...
	if user_id then
		...
		if identity then
			vRP.setUData(user_id,"Datatable",json.encode(vRP.user_tables[user_id]))
			TriggerClientEvent("vRP:showIds",-1,showIds)
			creativelogs(webhookjoins,"```[NOME]: "..identity.name.." "..identity.name2.." \n[ID]: "..user_id.." \n[ENTROU NO SERVIDOR] "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
		end
	end
end)