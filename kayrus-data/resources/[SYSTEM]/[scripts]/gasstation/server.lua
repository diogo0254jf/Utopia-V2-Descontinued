        ...
        for m, n in pairs(Config.gas_station_locations) do
            if not Config.gas_station_types[n.type] then
                if Config.lang == "br" then
                    print("^8[" ..GetCurrentResourceName() .."] Erro detectado no seu arquivo de configuracao, o tipo '" ..n.type .. "' nao esta cadastrado em Config.gas_station_types^7")
                else
                    print("^8[" ..GetCurrentResourceName() .."] Error detected in your configuration file, the type '" ..n.type .. "' is not registered in Config.gas_station_types^7")
                end
            end
        end
    end
...
RegisterServerEvent("gas_station:itemcarro")
AddEventHandler("gas_station:itemcarro",function(x)
local source = source
...
AddEventHandler(
    ...
    function(r, z, C, D, type, x)
        ...
        if x then
            ...
            if Config.ESX.account_stores == "money" or Config.ESX.account_stores == "cash" then
                vRP.giveDinheirama(s,tonumber(x.reward) or 0)
            else
                vRP.giveDinheirama(s,tonumber(x.reward) or 0)
            end
        ...
        if b[source] == nil then
            ...
            if t then
                ...
                if parseInt(query.user_id) == t then
                    ...
                    MySQL.Sync.execute(o, {["@gas_station_id"] = r})
                    if Config.ESX.account_stores == "money" or Config.ESX.account_stores == "cash" then
                        vRP.giveDinheirama(t,Config.gas_station_locations[r].sell_price)
                    else
                        vRP.giveDinheirama(t,Config.gas_station_locations[r].sell_price)
                    end
