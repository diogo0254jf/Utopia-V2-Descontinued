			...
			if distance <= 20.0 then
				...
				if distance <= 1.0 then
					if job_data[k] == nil then
						...
						if IsControlJustPressed(0,38) then
							if truck then
								TriggerEvent("gas_station:Notify","negado",Lang[Config.lang]['already_has_job'],5000)
								break
							end
							...
							if checkPos == false then
								TriggerEvent("gas_station:Notify","negado",Lang[Config.lang]['occupied_places'],5000)
								break
							end
...
AddEventHandler('gas_station:startContract', function(amount,truck_level,ressuply_id,type,query_delivery)
	...
	repeat
		...
		if cont >= (#Config.delivery_locations*2) then
			TriggerEvent("gas_station:Notify","negado",Lang[Config.lang]['not_found_location'],5000)
			return
		end
	...
	if Config.trucks[truck_level].trailer then
		...
		TriggerServerEvent("gas_station:itemcarro")
	end
	TriggerEvent("gas_station:Notify","sucesso",Lang[Config.lang]['already_is_in_garage'],5000)
	...
	while truck do
		...
		if IsEntityDead(ped) then
			...
			route_blip = nil
			PlaySoundFrontend(-1, "PROPERTY_PURCHASE", "HUD_AWARDS", 0)
			TriggerEvent("gas_station:Notify","negado",Lang[Config.lang]['you_died'],5000)
			return
		end
		...
		if engineH <= 150 then
			...
			route_blip = nil
			PlaySoundFrontend(-1, "PROPERTY_PURCHASE", "HUD_AWARDS", 0)
			TriggerEvent("gas_station:Notify","negado",Lang[Config.lang]['vehicle_destroyed'],5000)
			return
		end
