...
function cRP.toggleService()
	...
	if not inService then
		...
		if GetEntityModel(ped) == GetHashKey("mp_m_freemode_01") then
			...
			SetPedComponentVariation(ped,3,11,0,2) --maos
			SetPedComponentVariation(ped,4,28,11,2) --calcas
			SetPedComponentVariation(ped,5,-1,0,2) --mochila
			SetPedComponentVariation(ped,6,20,0,2) --sapatos
			SetPedComponentVariation(ped,7,21,1,2) --acessorios
			...
			SetPedComponentVariation(ped,11,13,3,2) --jaqueta
		elseif GetEntityModel(ped) == GetHashKey("mp_f_freemode_01") then
	
			SetPedComponentVariation(ped,1,-1,0,2) --mascara
			SetPedComponentVariation(ped,3,9,0,0) --maos
			SetPedComponentVariation(ped,4,34,0,2) --calcas
			SetPedComponentVariation(ped,5,-1,0,2) --mochila
			SetPedComponentVariation(ped,6,13,0,2) --sapatos
			SetPedComponentVariation(ped,7,-1,0,2) --acessorios
			SetPedComponentVariation(ped,8,15,0,2) --camisa
			SetPedComponentVariation(ped,9,-1,0,2) --coletes
			SetPedComponentVariation(ped,10,-1,0,2) --adesivos
			SetPedComponentVariation(ped,11,9,2,2) --jaqueta
		end
		inService = true
		makeBlipsPosition()
		...
		if inService then
			...
			if IsPedInAnyVehicle(ped) then
				...
				if distance <= 300 and IsVehicleModel(veh,GetHashKey("bus")) then
					timeDistance = 4
					DrawMarker(21,coords[driverPosition][1],coords[driverPosition][2],coords[driverPosition][3]+0.20,0,0,0,0,180.0,130.0,2.0,2.0,1.0,136, 96, 240, 180,1,0,0,1)
					if distance <= 15 then
