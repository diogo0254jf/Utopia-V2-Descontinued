	...
	if user_id then
		for k,v in pairs(itemList) do
			...
			if vRP.getInventoryItemAmount(user_id,v.item) >= parseInt(rand) then
				...
				if (v.item == "joint" or v.item == "cocaine") and math.random(100) >= 75 then
					local x,y,z = vRPclient.getPositions(source)
					local copAmount = vRP.numPermission("Police")
					for k,v in pairs(copAmount) do
						async(function()
							TriggerClientEvent("NotifyPush",v,{ time = os.date("%H:%M:%S - %d/%m/%Y"), code = 'QRU', title = "Denúncia de Venda de Drogas", x = x, y = y, z = z, rgba = {41,76,119} })
						end)
					end
				end
