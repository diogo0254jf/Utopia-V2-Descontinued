...
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local inLocate = { 195.45,-1764.04,29.36 }
...
Citizen.CreateThread(function()
	while true do
		...
		if distance <= 5 then
			timeDistance = 4
			DrawText3D(195.45,-1764.04,29.36,"~g~E~w~   DELIVERY")
			if distance <= 1 and IsControlJustPressed(1,38) then
				if inService then
					...
					inService = true
					TriggerEvent("Notify","verde","Serviço iniciado.",3000)
				end
			end
		end
	...
	while true do
		if inService and inTimers > 0 then
			...
			if inTimers <= 0 then
				...
				TriggerEvent("NotifyPush",{ code = "ENTREGA", title = "Quero comprar um produto", x = pedLocate[rand][1], y = pedLocate[rand][2], z = pedLocate[rand][3], name = callName[math.random(#callName)].." "..callName2[math.random(#callName2)], rgba = {69,115,41} })
			end
		end
