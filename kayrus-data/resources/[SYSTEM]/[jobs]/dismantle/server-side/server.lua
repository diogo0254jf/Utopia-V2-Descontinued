...
-- DESMANCHE
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("lRdismantle",function(source)
	local user_id = vRP.getUserId(source)
	if user_id then
		if timeList > 0 and userList[user_id] then
			...
			if vehListNames ~= "" then
				TriggerClientEvent("Notify",source,"verde",vehListNames.." a lista é atualizada em <b>"..parseInt(timeList).." minutos</b>, cada veículo entregue o mesmo é removido da lista.",10000)
			else
				TriggerClientEvent("Notify",source,"verde","Nenhum veículo encontrado na lista, aguarde <b>"..parseInt(timeList).." minutos</b>, até que esteja cheio de veículos novos.",10000)
			end
		end
	end
end)
