						...
						if inPackage then
							...
							if IsControlJustPressed(1,38) and boxVehicles[plate] then
								if boxVehicles[plate] > 0 then
									inPackage = true
									handPackage = true
									TriggerServerEvent("postop:remPackage",plate)
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- POSTOP:UPDATEPACKAGE
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("postop:updatePackage")
AddEventHandler("postop:updatePackage",function(status)
	boxVehicles = status
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- POSTOP:UPDATEPACKAGE
...
AddEventHandler("postop:initDelivery",function()
	...
	TriggerEvent("NotifyPush",{ code = 'ENTREGA', title = "Entrega de encomenda", x = delivery[posPackage][1], y = delivery[posPackage][2], z = delivery[posPackage][3], name = callName[math.random(#callName)].." "..callName2[math.random(#callName2)], rgba = {30,90,160} })
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- DWTEXT
-----------------------------------------------------------------------------------------------------------------------------------------
