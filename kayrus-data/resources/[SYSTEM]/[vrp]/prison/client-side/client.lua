...
Citizen.CreateThread(function()
	while true do
		...
		if prison then
			...
			if distance <= 10 then
				timeDistance = 4
				DrawText3D(services[prisonLocal][numServices][1],services[prisonLocal][numServices][2],services[prisonLocal][numServices][3],"~g~E~w~   CONSERTAR")
...
function cRP.startPrison(status)
	...
	if GetEntityModel(ped) == GetHashKey("mp_m_freemode_01") then
		...
		SetPedComponentVariation(ped,1,-1,5) --mascara
		SetPedComponentVariation(ped,3,0,0,2) --maos
		SetPedComponentVariation(ped,4,59,1,2) --calcas
		SetPedComponentVariation(ped,5,-1,0,2) --mochila
		SetPedComponentVariation(ped,6,25,0,2) --sapatos
		SetPedComponentVariation(ped,7,-1,0,2) --acessorios
		SetPedComponentVariation(ped,8,15,0,2) --camisa
		SetPedComponentVariation(ped,9,-1,0,2) --coletes
		SetPedComponentVariation(ped,10,-1,0,2) --adesivos
		SetPedComponentVariation(ped,11,22,1,2) --jaqueta
	elseif GetEntityModel(ped) == GetHashKey("mp_f_freemode_01") then

		SetPedComponentVariation(ped,1,-1,0,2) --mascara
		SetPedComponentVariation(ped,3,14,0,0) --maos
		SetPedComponentVariation(ped,4,61,1,2) --calcas
		SetPedComponentVariation(ped,5,-1,0,2) --mochila
		SetPedComponentVariation(ped,6,25,0,2) --sapatos
		SetPedComponentVariation(ped,7,-1,0,2) --acessorios
		SetPedComponentVariation(ped,8,15,0,2) --camisa
		SetPedComponentVariation(ped,9,-1,0,2) --coletes
		SetPedComponentVariation(ped,10,-1,0,2) --adesivos
		SetPedComponentVariation(ped,11,49,1,2) --jaqueta
	end
	prison = true
	prisonLocal = status
		...
		if prison then
			...
			if prisonLocal == 1 then
				if not mrpd:isPointInside(coords) then
					...
					TriggerEvent("Notify","vermelho","Nós achamos você tentando escapar.",5000)
				end
			else
...
-- PRISON:SWITCHHUD
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("prison:switchHud")
AddEventHandler("prison:switchHud",function(status)
	showHud = status
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- DWTEXT
