...
AddEventHandler("player:salary",function()
	...
	if user_id then
		...
		if vRP.hasPermission(parseInt(user_id),"Bronze") then
			vRP.addBank(parseInt(user_id),3000)
			TriggerClientEvent("Notify",source,"verde","Voce recebeu seu Benefício.",5000)
		end

		if vRP.hasPermission(parseInt(user_id),"Silver") then
			vRP.addBank(parseInt(user_id),5000)
			TriggerClientEvent("Notify",source,"verde","Voce recebeu seu Benefício.",5000)
		end

		if vRP.hasPermission(parseInt(user_id),"Gold") then
			vRP.addBank(parseInt(user_id),7000)
			TriggerClientEvent("Notify",source,"verde","Voce recebeu seu Benefício.",5000)
		end

		if vRP.hasPermission(parseInt(user_id),"Diamond") then
			vRP.addBank(parseInt(user_id),10000)
			TriggerClientEvent("Notify",source,"verde","Voce recebeu seu Benefício.",5000)
		end

		if vRP.hasPermission(parseInt(user_id),"Rubi") then
			vRP.addBank(parseInt(user_id),15000)
			TriggerClientEvent("Notify",source,"verde","Voce recebeu seu Benefício.",5000)
		end

		if vRP.hasPermission(parseInt(user_id),"Black") then
			vRP.addBank(parseInt(user_id),15000)
			TriggerClientEvent("Notify",source,"verde","Voce recebeu seu Benefício.",5000)
		end

		if vRP.hasPermission(parseInt(user_id),"Juridic") then
			vRP.addBank(parseInt(user_id),3000)
		...
		if vRP.hasPermission(parseInt(user_id),"Taxi") then
			vRP.addBank(parseInt(user_id),500)
			TriggerClientEvent("Notify",source,"verde","Voce recebeu seu salario.",5000)
		end
...
-- 	if user_id then
-- 		if not vCLIENT.getHandcuff(source) then
-- 			local service = vRP.prompt(source,"911: Polícia     |     112: Paramédico     |    411: Mecanico  ","")
-- 			if service == "" or (parseInt(service) ~= 911 and parseInt(service) ~= 411 and parseInt(service) ~= 112 ) then
-- 				return
-- 			end
...
-- 			elseif parseInt(service) == 411 then
...
-- 						if request then
-- 							TriggerClientEvent("NotifyPush",v,{ code = 'QTH', title = "Chamado", x = x, y = y, z = z, name = identity.name.." "..identity.name2, phone = identity.phone, rgba = {69,115,41} })
			...
			if vRPclient.getHealth(source) > 101 then
				if args[1] then
					...
					if plateUser then
						...
						if identity then
							vRPclient.playSound(source,"Event_Message_Purple","GTAO_FM_Events_Soundset")
							TriggerClientEvent("Notify",source,"amarelo","<b>Passaporte:</b> "..identity.id.."<br><b>RG:</b> "..identity.registration.."<br><b>Nome:</b> "..identity.name.." "..identity.name2.."<br><b>Telefone:</b> "..identity.phone,10000)
						end
					else
						...
						vRPclient.playSound(source,"Event_Message_Purple","GTAO_FM_Events_Soundset")
						TriggerClientEvent("Notify",source,"amarelo","<b>Passaporte:</b> "..plateSave[args[1]][1].."<br><b>RG:</b> "..string.upper(args[1]).."<br><b>Nome:</b> "..plateSave[args[1]][2].."<br><b>Telefone:</b> "..plateSave[args[1]][3],10000)
					end
				else
					...
					if vehicle then
						...
						if plateUser then
							...
							vRPclient.playSound(source,"Event_Message_Purple","GTAO_FM_Events_Soundset")
							TriggerClientEvent("Notify",source,"amarelo","<b>Passaporte:</b> "..plateSave[vehPlate][1].."<br><b>RG:</b> "..vehPlate.."<br><b>Nome:</b> "..plateSave[vehPlate][2].."<br><b>Telefone:</b> "..plateSave[vehPlate][3],10000)
						end
					end
				end
			end
	...
	if user_id then
		if not vCLIENT.getHandcuff(source) then
			...
			if distanceService then
				...
				if service == "Juridic" then
					if vRP.hasPermission(user_id,"Juridic") then
						vRP.removePermission(source,"Juridic")
						TriggerEvent("blipsystem:serviceExit",source)
						TriggerClientEvent("Notify",source,"amarelo","Você saiu de serviço.",5000)
						vRP.execute("vRP/upd_group",{ user_id = user_id, permiss = "Juridic", newpermiss = "waitJuridic" })
					elseif vRP.hasPermission(user_id,"waitJuridic") then
						vRP.insertPermission(source,"Juridic")
						TriggerEvent("blipsystem:serviceEnter",source,"Jurídico",80)
						TriggerClientEvent("Notify",source,"amarelo","Você entrou em serviço.",5000)
						vRP.execute("vRP/upd_group",{ user_id = user_id, permiss = "waitJuridic", newpermiss = "Juridic" })
					end
				end
				...
				if service == "Mechanic" then
					...
					elseif vRP.hasPermission(user_id,"waitMechanic") then
						...
						TriggerEvent("blipsystem:serviceEnter",source,"Mecânico",82)
						TriggerClientEvent("Notify",source,"amarelo","Você entrou em serviço.",5000)
			...
			if not vRP.hasPermission(user_id,"Police") then
				...
				if distance then
					...
					for k,v in pairs(comAmount) do
						async(function()
							TriggerClientEvent("NotifyPush",v,{ code = 'QRU', title = "Confronto em andamento", x = x, y = y, z = z, criminal = "Disparos de arma de fogo", rgba = {105,52,136} })
						end)
					end
				end
			end
...
function cRP.carryToggle()
	...
	local user_id = vRP.getUserId(source)
	if user_id then
		if vRP.hasPermission(user_id,"Police") or vRP.hasPermission(user_id,"Paramedic") or vRP.hasPermission(user_id,"Admin") or vRP.hasPermission(user_id,"Mod") then
			if vRPclient.getHealth(source) > 101 and not vCLIENT.getHandcuff(source) then
				local nplayer = vRPclient.nearestPlayer(source,2)
				if nplayer then
...
RegisterCommand("check",function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if user_id then
		if vRP.hasPermission(user_id,"Police") or vRP.hasPermission(user_id,"Paramedic") or vRP.hasPermission(user_id,"Police")then
...
-- SETREPOSE
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("setarepouso",function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if vRP.hasPermission(user_id,"Paramedic") then
		local nplayer = vRPclient.nearestPlayer(source,2)
		if nplayer then
...
RegisterCommand("onduty",function(source,args,rawCommand)
	...
	if user_id then
		if vRPclient.getHealth(source) > 101 and not vCLIENT.getHandcuff(source) then
			if vRP.hasPermission(user_id,"Police") or vRP.hasPermission(user_id,"Paramedic") or vRP.hasPermission(user_id,"Mechanic") or vRP.hasPermission(user_id,"Admin") then
				local onDuty = ""
				local service = {}
