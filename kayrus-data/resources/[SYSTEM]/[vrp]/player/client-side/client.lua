...
RegisterCommand("fps",function(source,args)
    if args[1] == "on" then
        ...
        TriggerEvent("Notify","verde","Boost de fps ligado!")
    elseif args[1] == "off" then
        SetTimecycleModifier("default")
        TriggerEvent("Notify","verde","Boost de fps desligado!")
    end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- GETHANDCUFF
	...
	while true do
		...
		if DoesEntityExist(ped) and GetEntityHealth(ped) > 101 and not IsPedInAnyVehicle(ped) then
			if CheckWeapon(ped) then
				if GetPedParachuteState(ped) < 1 then --parachute
				if not holster then
					timeDistance = 4
...
-----------------------------------------------------------------------------------------------------------------------------------------
local disService = {
	{ 312.96,-594.5,43.29,"Paramedic" }, -- Los Santos HP
	{ 442.87,-982.16,30.69,"Police" }, -- Los Santos DP
	{ -347.49,-133.29,39.01,"Mechanic" }, -- Los Santos Custom
	{ -139.51,-632.2,168.87, "Juridic" }  --Jurídico
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- DISTANCESERVICE
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler("player:CheckTrunk",function()
	...
	if inTrunk then
		...
		if DoesEntityExist(vehicle) then
			...
			inTrunk = false
			DetachEntity(ped,false,false)
			SetEntityInvisible(ped,true,false)
		...
		if inTrunk then
			...
			if DoesEntityExist(vehicle) then
				...
				if IsEntityVisible(ped) then
					SetEntityInvisible(ped,false,false)
				end
