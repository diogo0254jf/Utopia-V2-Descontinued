...
const updateDrag = () => {
	...
	$(".populated").tooltip({
		create: function(event,ui){
			...
			$(this).tooltip({
				content: `<item>${name}</item>${desc !== "undefined" ? "<br><description>"+desc+"</description>":""}<br><legenda>Tipo: <r>${type}</r> <s>|</s> Máximo: <r>${max !== "undefined" ? max:"S/L"}</r><br>Quantidade: <r>${formatarNumero(amounts)}</r> <s>|</s> Peso: <r>${(weight * amounts).toFixed(2)}</r></legenda>`,
