...
const updateGarages = () => {
	$.post("http://garages/myVehicles",JSON.stringify({}), data => {
		...
		// nameList.map(item => console.log(`http://127.0.0.1./cars_images/${item.name2}.png`));
		$("#displayHud").html(`
			<div id="vehList">
				${nameList.map((item) => (`
					<div class="card">
						<div class="vehicle" data-name="${item.name}">
							<div class="card-foto">
								...
								<img class="img-carros" src="http://191.252.205.38/fivem/nation_garages/${item.name}.png" onError="this.src='http://${IP}/vehicles/no-image.png';">
