...
RegisterCommand("garmas",function(source,args,rawCommand)
    ...
    if user_id then
        if vRPclient.getHealth(source) > 101 and not vCLIENT.getHandcuff(source) then
            ...
            TriggerClientEvent("Notify",source,"verde","Armamento guardado.",5000)
        end
    end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
		...
		for k,v in pairs(dropList) do
			for y,w in pairs(v) do
				if w[6] > 0 then
					...
					if w[6] <= 0 then
						dropList[k][y] = nil
						TriggerClientEvent("inventory:dropUpdates",-1,dropList)
			...
			if inv then
				...
				if vRP.itemTypeList(itemName) == "use" then
					if itemName == "bandage" then
						if vRPclient.getHealth(source) > 101 and vRPclient.getHealth(source) < 200 then
							...
							TriggerClientEvent("Notify",source,"azul","Você não pode utilizar de vida cheia ou nocauteado.",5000)
						end
					end
					...
					if itemName == "bonusDelivery" then
						...
						if vRP.tryGetInventoryItem(user_id,itemName,1,true,slot) then
							vRP.setBonusDelivery(user_id,5)
					...
					if itemName == "bonusPostOp" then
						...
						if vRP.tryGetInventoryItem(user_id,itemName,1,true,slot) then
							vRP.setbonusPostOp(user_id,5)
							TriggerClientEvent("Notify",source,"amarelo","O nível de experiência no <b>Entregador</b> aumentou.",5000)
						end
					end

					if itemName == "bonusGarbegeman" then
						local myBonus = vRP.bonusGarbageman(user_id)
						if parseInt(myBonus) >= 100 then
							return
						end

						if vRP.tryGetInventoryItem(user_id,itemName,1,true,slot) then
							vRP.setbonusGarbageman(user_id,5)
							TriggerClientEvent("Notify",source,"amarelo","O nível de experiência no <b>Lixeiro</b> aumentou.",5000)
						end
					end
                        ...
                        if not vRPclient.inVehicle(source) then
                            ...
                            if vehicle then
                                ...
                                if taskResult then
                                    ...
                                    TriggerClientEvent("Notify",source,"azul","Carro arrumado com Sucesso.",7000)
                                    vRP.removeInventoryItem(user_id,itemName,1,true)
								else
                                    TriggerClientEvent("Notify",source,"azul","Voce falhou.",7000)
                                end
						...
						if not vRPclient.inVehicle(source) then
							...
							if vehicle then
								...
								if taskResult then
									...
									TriggerClientEvent("inventory:repairVehicle",-1,vehNet,false)
							...
							if vRPclient.inVehicle(source) then
								...
								if taskResult then
									...
									if math.random(100) >= 75 then
										...
										vCLIENT.VehicleAlarmTimeLeft(vehicle,10000)
								...
								if status then
									...
									repeat
										if active[user_id] == 0 then
											...
											vRPclient._removeObjects(source)
											vCLIENT.blockButtons(source,false)
											vRP.giveInventoryItem(user_id,"dollars2",math.random(500,1000),true)
											...
											if math.random(100) >= 75 then
												...
												for k,v in pairs(copAmount) do
													async(function()
														TriggerClientEvent("NotifyPush",v,{ time = os.date("%H:%M:%S"), code = "QRU", title = "Roubo a Caixa Registradora", x = x, y = y, z = z, rgba = {170,80,25} })
													end)
												end
											end
										end
							...
							if vTASKBAR.taskFishing(source) then
								...
								if vRP.computeInvWeight(user_id) + vRP.itemWeightList(fishs[rand]) * rand <= vRP.getBackpack(user_id) then
									if vRP.tryGetInventoryItem(user_id,"bait",rand,true) then
										...
										TriggerClientEvent("Notify",source,"azul","Você precisa de <b>"..vRP.format(rand).."x "..vRP.itemNameList("bait").."</b>.",5000)
									end
								else
					...
					if itemName == "backpackp" then
						...
						if exp < 25 then
							...
							TriggerClientEvent("Notify",source,"azul","No momento você não pode usar essa mochila.",5000)
						end
					end
                        ...
                        vRP.upgradeStress(user_id,2)
                        	if vRP.tryGetInventoryItem(user_id,itemName,1,true,slot) then
                        		TriggerClientEvent("admin:vehicleTuning",source)
                       			TriggerClientEvent("Notify",source,"azul","Tech Aplicado com Sucesso.",7000)
                        	else
                       			TriggerClientEvent("Notify",source,"azul","vocé errou tente novamente.",7000)
                            end
                        end
                     end
                end
					...
					if itemName == "skate" then
						...
						vRP.tryGetInventoryItem(user_id,"skate",1,true)
					end

					if itemName == "shape" then
						if vRP.getInventoryItemAmount(user_id,"truck") >= 1 then
							if vRP.getInventoryItemAmount(user_id,"rolamento") >= 1 then
								TriggerClientEvent("Progress",source,10000)
								TriggerClientEvent("cancelando",source,true)
								vRPclient._createObjects(source,"anim@amb@clubhouse@tutorial@bkr_tut_ig3@","machinic_loop_mechandplayer","p_defilied_ragdoll_01_s",49,18905,0.08,-0.00,0.03,-150.0,90.0,-10.0)

								Citizen.Wait(10000)
								vRP.tryGetInventoryItem(user_id,"shape",1)
								vRP.tryGetInventoryItem(user_id,"truck",1)
								vRP.tryGetInventoryItem(user_id,"rolamento",1)
								vRPclient._stopAnim(source,false)
								vRPclient._removeObjects(source)
								TriggerClientEvent("cancelando",source,false)
								TriggerClientEvent("Notify",source,"verde","Pronto voce acabou de montar o seu skate.",7000)
								TriggerClientEvent("skate",source)

							else
								TriggerClientEvent("Notify",source,"vermelho","Voce precisa de um rolamento.",7000)
							end		
						else
							TriggerClientEvent("Notify",source,"vermelho","Voce precisa de um truck.",7000)
						end
					end
					...
					if has_value(pistols, weaponName) then
						for k,v in pairs(weaponsList) do
							if has_value(pistols, k) then
								TriggerClientEvent("Notify", source, "vermelho", "Você já possui uma arma desse tipo equipada.")
								return
							end
						end
					end
				...
				if vRP.itemTypeList(itemName) == "recharge" then
					...
					if checkWeapon then
						if vRP.tryGetInventoryItem(user_id,itemName,parseInt(rAmount),true,slot) then
							TriggerClientEvent("Progress",source,2000)
							TriggerClientEvent("Notify",source,"amarelo","VOCÉ RECARREGOU SUA ARMA.",2000)
							Citizen.Wait(2000)
...
AddEventHandler("vRP:playerSpawn",function(user_id,source)
	TriggerClientEvent("inventory:dropUpdates",source,dropList)
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- INVENTORY:CANCEL
-----------------------------------------------------------------------------------------------------------------------------------------
